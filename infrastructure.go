// Copyright (c) 2018 The GoChainRpc developers
package ripplerpc
...
type (
	...
	rawResponse struct {
		...
		Error  *xrpjson.RPCError `json:"error"`
	}
)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := xrpjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := xrpjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
